...
public class PaceListView extends LinearLayout {
    ...
    public PaceListView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void getDatas(){
        ...
        for(HistoryGPSPoint historyGPSPoint : list){
            ...
            if(historyGPSPoint.getTotal_length() / 1000 > currentKm){
                ...
                //2.计算平均配速
                paceChatBean.setAvg_pace_long(pace / count);
                ...
                if(minPace == 0){
                    minPace = paceChatBean.getAvg_pace_long();
                }else{
                    if(paceChatBean.getAvg_pace_long() != 0){
                        minPace = Math.min(minPace, paceChatBean.getAvg_pace_long());
                    }
                }
        ...
        mAdapter.setMaxDuration((int) maxDuration);
        mAdapter.setMinPace(minPace);
